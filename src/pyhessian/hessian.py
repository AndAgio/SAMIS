...
class Hessian():
    ...
    def eigenvalues(self, maxIter=100, tol=1e-3, top_n=1):
        ...
        while computed_dim < top_n:
            print('Computed {} out of {}. This may take a while...'.format(computed_dim, top_n))
            eigenvalue = None
            v = [torch.randn(p.size()).to(device) for p in self.params
                ]  # generate random vector
            v = normalization(v)  # normalize the vector

            for i in tqdm(range(maxIter), desc = "Running Power iteration eigen computation iterations"):
    ...
    def trace(self, maxIter=100, tol=1e-3):
        ...
        for i in tqdm(range(maxIter), desc = "Running Hutchinson trace approximation iterations"):
        # for i in range(maxIter):
            self.model.zero_grad()
            v = [
    ...
    def density(self, iter=100, n_v=1):
        ...
        for k in range(n_v):
            ...
            beta_list = []
            ############### Lanczos
            for i in tqdm(range(iter), desc = "Running Lanczos density estimation iterations"):
            # for i in range(iter):
                self.model.zero_grad()
