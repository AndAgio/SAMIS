...
class NaiveGreedy(optimizer):
    ...
    def select(self, gain_function, update_state=None, **kwargs):
        ...
        for _ in range(sum(selected), self.budget):
...
class ApproximateLazyGreedy(optimizer):
    def __init__(self, index, budget:int, already_selected=[], beta: float=0.9):
        super(ApproximateLazyGreedy, self).__init__(index, budget, already_selected)
        self.beta = beta
